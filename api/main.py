...
async def get_bot_status():
    ...
    try:
        ...
        uptime = (datetime.now() - bot_state["start_time"]).total_seconds() / 3600 if bot_state["start_time"] else 0
    ...
    try:
            ...
            with open(trades_file, 'r') as f:
                ...
                for trade in trades_data:
                    ...
                    if sym not in all_trades:
                        all_trades[sym] = {'wins': 0, 'total': 0, 'profit': 0}
                    all_trades[sym]['total'] += 1
                    if trade.get('profit', 0) > 0:
                        all_trades[sym]['wins'] += 1
                    all_trades[sym]['profit'] += trade.get('profit', 0)
        ...
        for symbol in symbols:
            ...
            stats.append(SymbolStats(
                symbol=symbol,
                status="ACTIVE",
                current_price=float(info.ask),
                bid=float(info.bid),
                ask=float(info.ask),
                spread_pts=spread,
                open_trades=open_by_symbol.get(symbol, 0),
                win_rate=win_rate,
